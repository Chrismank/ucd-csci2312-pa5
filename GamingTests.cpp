...
//        
        ...
        
        {
            ...
            std::cout << "NUM AGENTS: " << g.getNumAgents() << std::endl;
            std::cout << "NUM RESOURCES: " << g.getNumResources() << std::endl;
...
void test_game_print(ErrorContext &ec, unsigned int numRuns) {
    ...
    for (int run = 0; run < numRuns; run++) {
        ...
        
        {
            ...
            for (int i = 0; i < 6; i++) {
                getline(ss, line);
                std::regex re1("(\\[([[:alpha:]]{1}[[:d:]]{1,4}[ ]?|[ ]{5})\\]){7}"); // one alphabetic character, followed by a number that could be up to four digits, 5 spaces
                std::regex_search(line, m, re1);
...
//            
//            std::cout << "Resources: " << g.getNumResources() << std::endl;
//            std::cout << "Agents: " << g.getNumAgents() << std::endl;
//            
//            pass = (g.getNumResources() == 0) &&
//            (g.getNumAgents() == 1);
//            
